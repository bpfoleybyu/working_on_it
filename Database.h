...
public:
    ...
    set<Table> evalRules(vector<rule> &rules); //this builds a relation for each rule on the rhs. preps them to be joined.
    ...
    set<Table> doRule(string name, vector<string> &queries); // does all of the work for the first rule    
    ...
    void ruleParam(vector<pair<int, int>> &matchPairs, vector<pair<int, string>> &pairs, vector<int> &projects, vector<string> &renames, vector<string> &tempQ, string name);
